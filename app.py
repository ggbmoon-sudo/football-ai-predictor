...
def deep_analyze_agent(match_name):
    ...
    🎯 目標賽事：{match_name}
    🚨【極重要時區警告】：上述標示的時間為「香港時間 (HKT, UTC+8)」。歐洲當地時間或國際莊家盤口（如 Bet365）通常標示為 UTC 或當地時區（比香港慢 7-8 小時）,並年度一定是現在(2026年)。
    👉 檢索防呆指令：在聯網搜尋賠率、傷停與賽事資訊時，請務必以「對陣球隊名稱」為主，並自行將 (HKT, UTC+8) 換算回歐洲當地時間。絕對不可因為時區對不上而判定「無此賽事」或胡亂編造數據！若真的查無此賽，請直接中止分析並說明。
