...
def deep_analyze_agent(match_name):
    ...
    🚨【極重要系統強制設定 - 必讀】：
    1. 雙時區基準：上述標題已同時提供「香港時間 HKT」與「國際協調時間 UTC」。請在聯網檢索賠率與歐洲當地新聞時，優先使用 UTC 時間對照，或直接依賴對陣球隊名稱。絕對不可因為時區差異而說找不到比賽！
    2. 賽事絕對存在 (無視升降級迷思)：這場比賽已經被官方 API 確認存在！絕對不可因為你內部的歷史數據（例如認定某隊已降級）而質疑這場比賽。無條件接受他們目前正在這個聯賽對戰的事實！
    3. 禁止罷工：絕對不允許以「查無此賽事」或「聯賽級別矛盾」為由中止分析。請直接利用你所能找到的最新戰術特徵、傷停情報與歷史 H2H 數據，強制完成推演！

    請聯網檢索最新資訊，並執行以下深度的四步推演邏輯，任何敷衍都將不被接受：

    第一步：【進球與失球模型 (xG/xGA) 拆解】
    - 檢索雙方近期的 xG (預期進球) 與 xGA (預期失球)，對比實際進球數。判斷哪隊把握力強，哪隊後防有隱患。

    第二步：【戰術相剋與陣容推演】
    - 檢索近期傷停情報。主隊的進攻發起方式是否剛好剋制客隊的防守弱點？

    第三步：【戰意與莊家資金劇本】
    - 判斷雙方戰意（保級/晉級/盃賽）。
    - 預估各大莊家（Bet365）的大細盤與讓球盤走勢，暗示了這是一場「沉悶防守戰」還是「對攻大戰」？

    第四步：【角球矩陣推算 (Corner Matrix)】
    - 根據控球率預測、邊路進攻依賴度（下底傳中頻率），推算本場總角球數與分佈。

    【最終精算結論】
    請綜合以上變數，給出最具說服力的理據，並在報告最末端嚴格以獨立的行數輸出以下標籤（勿加其他符號）：
    [Score: X-Y]
    [Corners: X-Y]
    [Rec: 推薦投注選項]
    [Win_Conf: X%]
