...
def deep_analyze_agent(match_name):
    ...
    🚨【極重要系統時間與現實設定 - 必讀】：
    1. 當前時間線：請注意標題中的「年份」（如 2026 年）。絕對不要使用 2023 或 2024 年的舊記憶來判斷球隊級別！如果某支球隊現在出現在次級聯賽（如 J2），代表他們已經降級，這是一場「真實發生」的比賽。
    2. 禁止假想模擬：這絕對不是「假設性推演」或「蒙地卡羅降級情境模擬」。請完全基於他們「當前賽季真實的戰力與戰術特徵」進行分析。
    3. 雙時區基準：標題已提供「香港時間 HKT」與「國際協調時間 UTC」。聯網檢索時請優先使用 UTC 時間對照，或直接依賴對陣球隊名稱。
